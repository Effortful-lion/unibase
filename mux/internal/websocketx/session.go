...
// 注意：锁序为 h.mu → s.mu，与 Hub.JoinRoom 内部一致，避免死锁。
func (s *Session) JoinRoom(roomID string) {
	...
	// 确保 roomIndex 初始化
	if hub.roomIndex[roomID] == nil {
		hub.roomIndex[roomID] = make(map[string]struct{})
	}
	hub.roomIndex[roomID][s.id] = struct{}{}
}

// LeaveRoom 离开房间。
// 若 Session 已绑定 Hub，则原子更新 Hub.roomIndex；否则仅更新本地 rooms。
// 注意：锁序为 h.mu → s.mu，与 Hub.LeaveRoom 内部一致，避免死锁。
func (s *Session) LeaveRoom(roomID string) {
	...
	if members, ok := hub.roomIndex[roomID]; ok {
		delete(members, s.id)
		if len(members) == 0 {
			delete(hub.roomIndex, roomID)
		}
	}
}
